...
const ContextualFeedbackPage = () => {
  ...
  const { request: validateTokenApi, error: apiErrorHook, isLoading: apiIsLoadingHook } = useApi();
  ...
  useEffect(() => {
     const validateSession = async () => {
        ...
        if (requiresToken && !token) {
            setValidationError(`Invalid feedback link: Token required for '${loc}' location.`);
           setIsLoading(false);
            return;
         }
         ...
         if (requiresToken && token) {
           ...
           const config = {
             ...
             url: `${API_BASE_URL}/feedback/validate-token?tok=${token}&loc=${loc}${id ? `&id=${id}` : ''}`, // Pass context to backend
           };
           const validationResponse = await validateTokenApi(config);
            ...
            if (validationResponse?.valid) {
               ...
               setContextData({ loc, id, token, ...(validationResponse.context || {}) });
               setIsValidSession(true);
            } else {
                 // Handle specific errors from backend if available, otherwise use hook error or generic
                const errMsg = validationResponse?.message || apiErrorHook || "Invalid or expired feedback session token.";
                setValidationError(errMsg);
                 setIsValidSession(false);
             }
         } else {
            // Public location or no token required, proceed directly
             console.log(`Public location '${loc}', proceeding without token validation.`);
        ...
        return (
           <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-light p-4 text-center">
               <Alert type="error" title="Invalid Link" message={validationError} />
                <p className="mt-4 text-sm text-neutral-dark">Please ensure you scanned the correct QR code. If the problem persists, contact reception.</p>
    ...
    if (isValidSession) {
       ...
       if (FeedbackFormComponent) {
           ...
           // Fallback if location code is unrecognized but passed initial checks
            return (
                <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-light p-4 text-center">
                     <Alert type="error" title="Error" message={`Unsupported feedback location code: ${loc}.`} />
                 </div>
             );
       }
   }
