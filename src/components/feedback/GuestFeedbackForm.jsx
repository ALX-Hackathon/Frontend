        ...
        <div className="mb-4">
          <label
            className={`block text-sm font-medium mb-2 ${
              error ? "text-error" : "text-neutral-darker"
            }`}
          >
          ...
          <div className="flex space-x-1.5 items-center">
            ...
            {value > 0 && (
              <span className="text-sm text-neutral-dark ml-2">({value})</span>
            )}
          </div>
          {error && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
        ...
        <fieldset className="border border-neutral p-4 rounded-md">
          ...
          <StarRatingInput
            ...
            rules={{
              ...
              min: { value: 1, message: "Please provide a rating" },
            }}
          />
          <Select
            id="feedbackType"
        ...
        <fieldset className="border border-neutral p-4 rounded-md space-y-4">
          ...
          <StarRatingInput
            name="diningService"
            control={control}
            label="Service (Restaurant/Bar Staff)"
          />
          <Textarea
            id="diningComments"
            label="Comments about Dining"
            rows={3}
            {...register("diningComments", {
              maxLength: { value: 500, message: "Max 500 characters" },
            })}
        ...
        <fieldset className="border border-neutral p-4 rounded-md space-y-4">
          ...
          <StarRatingInput
            name="wifiRating"
            control={control}
            label="Wi-Fi Quality/Speed"
          />
          <StarRatingInput
            name="poolRating"
            control={control}
            label="Swimming Pool (if applicable)"
          />
          <StarRatingInput
            name="restroomRating"
            control={control}
            label="Public Restroom Cleanliness"
          />
        ...
        <fieldset className="border border-neutral p-4 rounded-md space-y-4">
          ...
          <StarRatingInput
            ...
            control={control}
            label="Staff Friendliness/Attitude"
          />
          <StarRatingInput
            name="receptionRating"
            control={control}
            label="Reception / Check-in / Check-out Process"
          />
        ...
        <fieldset className="border border-neutral p-4 rounded-md">
          <legend className="text-base font-medium px-2 text-neutral-darker mb-2">
            Attach Photos (Optional)
          </legend>
          <p className="text-xs text-neutral-dark mb-3">
            If reporting an issue, attaching photos can help (Max 3 files,
            JPG/PNG). Files are NOT actually uploaded in this demo.
          </p>
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedFiles.map((file, index) => (
                <div
                  key={index}
                  className="flex items-center gap-1 bg-neutral-light px-2 py-1 rounded text-xs text-neutral-darker border border-neutral"
                >
                  <span>
                    {file.name.length > 15
        ...
        <div className="relative flex items-start mt-4">
          ...
          <div className="ml-3 text-sm">
            <label
              ...
              className="font-medium text-neutral-darker"
            >
              Contact Consent
            </label>
            <p
              id="contactConsent-description"
              className="text-neutral-dark text-xs"
            >
              Check this box if you would like to allow hotel management to
              contact you regarding your feedback (if necessary).
            </p>
          </div>
        </div>
