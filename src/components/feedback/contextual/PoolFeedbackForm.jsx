...
const PoolFeedbackForm = ({ context }) => {
  ...
  const areaName =
    context.loc === "pool"
      ...
      : context.id
      ? `Area: ${context.id}`
      : "Hotel Amenities"; // Customize title based on context
  ...
  const onSubmit = async (data) => {
    const payload = {
      ...
      context,
      source: "Guest",
      feedbackArea: context.loc || "Amenity",
    }; // Use loc as feedbackArea
    const config = {
      method: "post",
  ...
  return (
    <Card title={`${areaName} Feedback`}>
      <p className="text-sm text-neutral-dark mb-4">
        Please share your experience regarding the {areaName.toLowerCase()}.
      </p>
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        <StarRatingInputHelper
          name="poolCleanliness"
          control={control}
          label="Cleanliness (Water & Area)"
          rules={{ required: true, min: 1 }}
        />
        <StarRatingInputHelper
          name="seatingAvailability"
          control={control}
          label="Availability of Chairs / Cabanas"
        />
        <StarRatingInputHelper
          name="towelAvailability"
          control={control}
          label="Towel Availability & Quality"
        />
        <StarRatingInputHelper
          name="poolAmbiance"
          control={control}
          label="Overall Atmosphere"
        />
        <Textarea
          id="amenityComments"
