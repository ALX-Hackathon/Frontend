...
const CheckoutFeedbackForm = ({ context }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if (successData !== null) {
      // Check if request was successful (useApi returns data or null on error)
      ...
      toast.error(
        "Failed to submit feedback. Please try again or contact reception."
      );
    }
  };
  ...
  return (
    <Card title={`Check-out Feedback${guestName}`}>
      <p className="text-sm text-neutral-dark mb-4">
        Thank you for staying with us! Please rate your recent check-out
        experience. (<span className="text-error">*</span> indicates required
        fields)
      </p>
      <form
      ...
      >
        ...
        <StarRatingInputHelper
          ...
          rules={{
            ...
            min: { value: 1, message: "Rating required" },
          }}
        />
