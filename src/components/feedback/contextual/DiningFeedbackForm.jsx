...
const DiningFeedbackForm = ({ context }) => {
    ...
    const locationIdentifier = context.id ? `at ${context.id}` : "in our dining area";

     const onSubmit = async (data) => {
         const payload = { ...data, context, source: 'Guest', feedbackArea: 'Dining' };
        const config = { method: 'post', url: `${API_BASE_URL}/feedback/contextual`, data: payload };
         const success = await submitFeedback(config);
         if (success) {
            toast.success(`Thank you for your feedback about dining ${locationIdentifier}!`);
            reset();
         } else {
    ...
    return (
        <Card title={`Dining Feedback (${locationIdentifier})`}>
            <p className="text-sm text-neutral-dark mb-4">How was your dining experience {locationIdentifier}? Please rate the following:</p>
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
                 {/* Use rules={{required: true, min: 1}} where applicable */}
                 <StarRatingInputHelper name="foodQuality" control={control} label="Food Quality" rules={{ required: true, min: 1 }} />
                <StarRatingInputHelper name="serviceSpeed" control={control} label="Speed of Service" />
                <StarRatingInputHelper name="staffAttentiveness" control={control} label="Staff Attentiveness / Friendliness" rules={{ required: true, min: 1 }} />
                <StarRatingInputHelper name="ambianceRating" control={control} label="Ambiance / Atmosphere" />
                <Textarea
                     id="diningComments"
