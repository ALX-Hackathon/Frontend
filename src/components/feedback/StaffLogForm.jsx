...
function StaffLogForm() {
  ...
  const validateForm = () => {
    ...
    if (details.trim().length > 1000)
      errors.details = "Details cannot exceed 1000 characters.";
  ...
  const handleSubmit = async (e) => {
    ...
    const config = {
      method: "post",
      url: `${API_BASE_URL}/feedback/staff`,
      data: payload,
    };
  ...
  return (
    <Card title="Log Verbal/Observed Feedback">
      <form onSubmit={handleSubmit} noValidate>
        ...
        />
        {formErrors.details && (
          <p id="details-error" className="sr-only">
            {formErrors.details}
          </p>
        )}
